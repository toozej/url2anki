...
verify: ## Verify Docker image with Cosign
	cosign verify \
		--certificate-identity-regexp '^https://github.com/toozej/url2anki/\.github/workflows/(release|weekly-docker-refresh)\.yaml@refs/(tags/.*|heads/main)$$' \
...
distroless-run: ## Run built Docker image using distroless as final base
	docker run --rm --name url2anki --env-file $(CURDIR)/.env $(IMAGE_AUTHOR)/$(IMAGE_NAME):$(IMAGE_TAG)-distroless
